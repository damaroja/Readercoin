...
// Copyright (c) 2009-2020 The Readercoin Core developers
...
static const uint32_t WALLET_RDC_KB_TO_SAT_B = COIN / 1000; // 1 sat / B = 0.00001 RDC / kB
...
static void SetFeeEstimateMode(const CWallet* pwallet, CCoinControl& cc, const UniValue& estimate_mode, const UniValue& estimate_param)
{
    ...
    if (cc.m_fee_mode == FeeEstimateMode::RDC_KB || cc.m_fee_mode == FeeEstimateMode::SAT_B) {
        ...
        if (cc.m_fee_mode == FeeEstimateMode::SAT_B) {
            fee_rate /= WALLET_RDC_KB_TO_SAT_B;
        }
...
static UniValue getnewaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getnewaddress",
                ...
                RPCResult{
                    RPCResult::Type::STR, "address", "The new readercoin address"
                },
                RPCExamples{
...
void ParseRecipients(const UniValue& address_amounts, const UniValue& subtract_fee_outputs, std::vector<CRecipient> &recipients) {
    ...
    for (const std::string& address: address_amounts.getKeys()) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Readercoin address: ") + address);
        }
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::ARR, "", "",
                        {
                            {RPCResult::Type::ARR, "", "",
                            {
                                {RPCResult::Type::STR, "address", "The readercoin address"},
...
static UniValue addmultisigaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "The readercoin addresses or hex-encoded public keys",
                        {
            ...
            RPCHelpMan{"listtransactions",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                            {RPCResult::Type::STR, "address", "The readercoin address of the transaction."},
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address", "The readercoin address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "The readercoin addresses to filter",
                        {
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS) && !want_psbt) {
        if (!pwallet->chain().rpcEnableDeprecated("bumpfee")) {
            throw JSONRPCError(RPC_METHOD_DEPRECATED, "Using bumpfee with wallets that have private keys disabled is deprecated. Use psbtbumpfee instead or restart readercoind with -deprecatedrpc=bumpfee. This functionality will be removed in 0.22");
        }
        want_psbt = true;
    }
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
            RPCHelpMan{"getaddressinfo",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The readercoin address for which to get information."},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::STR, "address", "The readercoin address validated."},
