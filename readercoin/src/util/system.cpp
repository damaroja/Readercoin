...
const char * const READERCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/readercoin/readercoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //readercoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", READERCOIN_SETTINGS_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Readercoin Core copyright is not removed by accident
    if (copyright_devs.find("Readercoin Core") == std::string::npos) {
