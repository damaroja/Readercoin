*After branching off for a major version release of Readercoin Core, use this
...
https://github.com/readercoin-core/readercoin-devwiki/wiki/
...
Readercoin Core version *version* is now available from:
  ...
  <https://readercoincore.org/bin/readercoin-core-*version*/>
...
RPC commands have been updated to include two new fee estimation methods "RDC/kB" and "sat/B".
...
- A new `readercoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the
  former `generate` RPC. See the help for details. (#19133)

- The `readercoin-cli -getinfo` command now displays the wallet name and balance for
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `readercoin-wallet` tool which performs the salvage
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/readercoin/readercoin/issues).
...
Users who already have a Readercoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which is the default type of wallet) will
behave as they did in previous versions of Readercoin Core.
...
Default Wallet created upon first startup of Readercoin Core will be a Legacy Wallet.
...
The change to using descriptors changes the default derivation paths used by Readercoin Core
