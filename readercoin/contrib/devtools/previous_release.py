...
def download_binary(tag, args) -> int:
    if Path(tag).is_dir():
        ...
        shutil.rmtree(tag)
    Path(tag).mkdir()
    bin_path = 'bin/readercoin-core-{}'.format(tag[1:])
    match = re.compile('v(.*)(rc[0-9]+)$').search(tag)
    if match:
        bin_path = 'bin/readercoin-core-{}/test.{}'.format(
            match.group(1), match.group(2))
    tarball = 'readercoin-{tag}-{platform}.tar.gz'.format(
        tag=tag[1:], platform=args.platform)
    sha256Sums = "SHA256SUMS-{tag}.asc".format(tag=tag[1:])
    tarballUrl = 'https://readercoincore.org/{bin_path}/{tarball}'.format(
        bin_path=bin_path, tarball=tarball)
    sha256SumsUrl = 'https://readercoincore.org/{bin_path}/SHA256SUMS.asc'.format(
        bin_path=bin_path)
    ...
    # Readercoin Core Release Signing Keys v0.11.0+
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          'readercoin-{tag}'.format(tag=tag[1:])]).returncode
    if ret:
        return ret
...
def build_release(tag, args) -> int:
    githubUrl = "https://github.com/readercoin/readercoin"
    if args.remove_dir:
        if Path(tag).is_dir():
            shutil.rmtree(tag)
    ...
    with pushd(tag):
        ...
        files = ['readercoind', 'readercoin-cli', 'readercoin-tx']
        for f in files:
